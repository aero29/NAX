...
for counter in range(totalrows):      # start counter from 1,totalaxirows+1 for only axisymmetric case
    ...
    if totalfiles == 1:
     ...
     # write non-axisymmteric geomturbo file
            ...
            for i in range(index,(index+totalfiles)):
                ...
                if i == int(index) :
                    ...
                    with open(sfiles[i]) as f_geom:
                        ...
                        for s1 in range(98,107):
                            f2.write(lineread[s1])
        ##                    
                        m2=108
                        f2.write(("    NAME     Blade-1") + "\n")
        ##               
                        for s1 in range(108,121):
                            f2.write(lineread[s1])
        ...
        #               
                        for s1 in range(122,file_length-5):
                            f2.write(lineread[s1])
                        f2.close()
                    ...
                    with open(sfiles[i]) as f_geom:
                        ...
                        for s1 in range(122,file_length):   
                            f4.write(lineread[s1])
                        f4.close() 
...
with open('igv643.geomTurbo', 'w') as outfile:   # change final geomTurbo file name here 
    for fname in files2:
